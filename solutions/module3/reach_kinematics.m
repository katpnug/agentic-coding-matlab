...
x_aligned = nan(sum(keep), 601);
kept_idx = find(keep);
for i = 1:numel(kept_idx)
    x_aligned(i, :) = ReachS(kept_idx(i)).filtered(:, 2)';
end

fig = figure('Color', 'w');
if isprop(fig, 'Theme'), fig.Theme = 'light'; end   % consistent colors on dark-mode MATLAB (R2025a+)
plot(t_rel, x_aligned', 'Color', [0.7 0.7 0.7 0.4]);
hold on;
plot(t_rel, mean(x_aligned, 1, 'omitnan'), 'k', 'LineWidth', 2.5);
hold off;
xlabel('time from window center (s)');
ylabel('hand x-position (m)');
...
mean_trace = @(sel) mean(cell2mat(arrayfun(@(x) x.filtered(:, 2), ...
